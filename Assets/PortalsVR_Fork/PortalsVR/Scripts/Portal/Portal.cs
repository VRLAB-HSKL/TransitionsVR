    ...
    public class Portal : MonoBehaviour
    {
        ...
        private void Awake()
        {
            ...
            foreach (Camera.StereoscopicEye eye in portalInfo.Keys)
            {
                ...
                // Custom texture resolution because XRSettings is not being initialized...
                var xDim = 1920;
                var yDim = 1048;

                var widthLoaded = XRSettings.eyeTextureWidth != 0;
                var heightLoaded = XRSettings.eyeTextureHeight != 0;
                
                var eyeTextureWidth = widthLoaded ? XRSettings.eyeTextureWidth : xDim;
                var eyeTextureHeight =  heightLoaded ? XRSettings.eyeTextureHeight : yDim;

                if (widthLoaded || heightLoaded)
                {
                    Debug.LogWarning(
                        "Unity.XR.XRSettings was not initialized! Custom eye texture dimensions set: " +
                        "(" + eyeTextureWidth + ", " + eyeTextureHeight + "). " +
                        "Try to set this to single eye lens resolution of your HMD if visual bugs start happening!"
                    );
                }
                
                
                
                portalInfo[eye].viewTexture = new RenderTexture(eyeTextureWidth, eyeTextureHeight, 24);
